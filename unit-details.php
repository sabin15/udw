<<<<<<< HEAD
<?php require('header.php') ?>

    <div class="jumbotron unit-header">
      <h2> Python OOPS: Object Oriented Programming </h2>
      <p> IT 1110 </p>
      <p>Learn Object Oriented Programming in Python With Concepts</p>
      <p><b>Unit Co-ordinator </b>: Prashant Pokhrel &ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp; <b>Lecturer </b>: Sabin Shrestha  </p>
    </div>
    <div class=" row">
      <div class="col-lg-1"></div>
      <div class="col-lg-6 unit-details">
        <h2>Description</h2>
        <p>Welcome to Object Oriented Programming With Python, One single course to start your Object Oriented Journey from Step-by-Step, This course touches each and every important concept of OOPS with it's latest version Python 3.7.</p>
        <p>OOP is one of the most important concept of programming, It is important because <br>
          1. Makes Visualization easier (Closest to real world scenarios) <br>
          2. Reuse the code, Saves time, Reduce lines of code, Shrinks your project.<br>
          3. Makes code Readable by adding a structure to data with the related methods for this data.<br>
          4. Modularity for easier troubleshooting <br>
          5. Reuse of code through inheritance <br>
          6. Flexibility through Polymorphism
        </p>
        <h3>Available Semesters</h3>
        <p><ol>
          <li>Semester 1</li>
          <li>Semester 2</li>
          <li>Winter School</li>
          <li>Spring School</li>
        </ol>
        </p>
        <h3>Available Campuses</h3>
        <p><ol>
          <li>Pandera</li>
          <li>Rivendell</li>
          <li>Neverland</li>

        </ol>
        </p>
      </div>
      <div class="col-lg-1"></div>
      <div class="col-lg-3 enroll-box">
        <p>Please get enrolled before classes are full.</p>
        <form method="get" action="enrollment.php">
            <button type="submit" class="btn btn-danger enroll-btn">Enroll Now</button>
        </form>

      </div>


    </div>

<?php require('footer.php') ?>
=======
<?php require('header.php') ?>

    <div class="jumbotron unit-header">
      <h2> Python OOPS: Object Oriented Programming </h2>
      <p> IT 1110 </p>
      <p>Learn Object Oriented Programming in Python With Concepts</p>
      <p><b>Unit Co-ordinator </b>: Prashant Pokhrel &ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp; <b>Lecturer </b>: Sabin Shrestha  </p>
    </div>
    <div class=" row">
      <div class="col-lg-1"></div>
      <div class="col-lg-6 unit-details">
        <h2>Description</h2>
        <p>Welcome to Object Oriented Programming With Python, One single course to start your Object Oriented Journey from Step-by-Step, This course touches each and every important concept of OOPS with it's latest version Python 3.7.</p>
        <p>OOP is one of the most important concept of programming, It is important because <br>
          1. Makes Visualization easier (Closest to real world scenarios) <br>
          2. Reuse the code, Saves time, Reduce lines of code, Shrinks your project.<br>
          3. Makes code Readable by adding a structure to data with the related methods for this data.<br>
          4. Modularity for easier troubleshooting <br>
          5. Reuse of code through inheritance <br>
          6. Flexibility through Polymorphism
        </p>
        <h3>Available Semesters</h3>
        <p><ol>
          <li>Semester 1</li>
          <li>Semester 2</li>
          <li>Winter School</li>
          <li>Spring School</li>
        </ol>
        </p>
        <h3>Available Campuses</h3>
        <p><ol>
          <li>Pandera</li>
          <li>Rivendell</li>
          <li>Neverland</li>

        </ol>
        </p>
      </div>
      <div class="col-lg-1"></div>
      <div class="col-lg-3 enroll-box">
        <p>Please get enrolled before classes are full.</p>
        <form method="get" action="enrollment.php">
            <button type="submit" class="btn btn-danger enroll-btn">Enroll Now</button>
        </form>

      </div>


    </div>

<?php require('footer.php') ?>
>>>>>>> 5b52427de29db2ac3ba3be035e5ae77260904766
